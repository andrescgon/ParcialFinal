...
linear_regression
    : LINEAR_REGRESSION OPAR expr COMMA expr CPAR
    ;
    
mlp_expr
 : MLP OPAR (mlp_train | mlp_predict) CPAR
 ;

mlp_train
 : TRAIN COMMA expr COMMA expr        // Datos de entrenamiento (X, y)
 COMMA LAYERS ASSIGN expr             // Configuración de capas
 COMMA EPOCHS ASSIGN expr             // Epochs como expresión
 COMMA LEARNING_RATE ASSIGN expr      // Learning rate como expresión
 ;


mlp_predict
 : PREDICT COMMA expr COMMA expr      // Modelo y datos
 ;
 
 
clustering
 : CLUSTER OPAR method=ID COMMA expr (COMMA params=objeto)? CPAR
 ;

clusteringPredict
 : CLUSTER_PREDICT OPAR expr COMMA expr CPAR
 ;
 
plot
 : PLOT OPAR expr COMMA expr CPAR NEWLINE?
 ;
...
assignment
    : variable ASSIGN (linear_regression | mlp_expr | clustering | clusteringPredict | assignment | expr)
    ;
...
accessarray
 : variable OKEY expr CKEY             // Explicit array access rule
 ;
...
expr
 ...
 | atom                                         #atomExpr
 | accessarray                                  #accessArrayExpr  // Cambio clave
 | agente                                       #agenteExpr
