...
class TLONGlobalMemory__():
  ...
  def __init__(self, local_memories=[]):
    ...
    self.register_predefined_functions()
    
  def register_predefined_functions(self):
        """Registrar funciones matemáticas y otras en la memoria global"""
        global_mem = self.memory_stack[0]  # Obtener la memoria principal

        # Agregar funciones matemáticas predefinidas
        global_mem.assign("sin", TLONVariable__("sin", math.sin, kind="default"), None)
        global_mem.assign("cos", TLONVariable__("cos", math.cos, kind="default"), None)
        global_mem.assign("tan", TLONVariable__("tan", math.tan, kind="default"), None)
        global_mem.assign("loga", TLONVariable__("loga", math.log, kind="default"), None)
        global_mem.assign("exp", TLONVariable__("exp", math.exp, kind="default"), None)
        global_mem.assign("sqrt", TLONVariable__("sqrt", math.sqrt, kind="default"), None)
        global_mem.assign("csc", TLONVariable__("csc", lambda x: 1 / math.sin(x), kind="default"), None)
        global_mem.assign("sec", TLONVariable__("sec", lambda x: 1 / math.cos(x), kind="default"), None)
        global_mem.assign("cot", TLONVariable__("cot", lambda x: 1 / math.tan(x), kind="default"), None)
        global_mem.assign("asin", TLONVariable__("asin", math.asin, kind="default"), None)
        global_mem.assign("acos", TLONVariable__("acos", math.acos, kind="default"), None)
        global_mem.assign("atan", TLONVariable__("atan", math.atan, kind="default"), None)
        global_mem.assign("relu", TLONVariable__("relu", lambda x: max(0, x), kind="default"), None)
  ...
  def get(self, name):
    var = self.find(name)
    if var is None:
        raise NameError(f"Variable '{name}' no encontrada")
